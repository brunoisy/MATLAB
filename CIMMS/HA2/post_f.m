...
%draw from prior
theta(1) = 2/vartheta;%gamrnd(2,1/vartheta);
lambda(:,1) = 2/theta(1);%gamrnd(2,1/theta(1),1,d);
t(:,1) = linspace(1851,1963,d+1);

for k = 1:(M-1);
    %draw N thetas from posterior using Gibbs
    theta(k+1) = gamrnd(2*d+2,1/(vartheta+sum(lambda(:,k)))) ;
    ...
    cand = [t(1,k); (t(2:end-1,k) + 2*R.*rand([d-1,1])-R); t(end,k)]; %Candidate vector incl fixed start/endpoints
