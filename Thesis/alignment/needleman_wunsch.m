function [score, alignment1, aligner, alignment2] = needleman_wunsch(seq1, seq2, matchA, matchU, mismatch, gapOpen, extendGap)
...
for i = 2:size(F,1) %row
    for j = 2:size(F,2) %col
        ...
        if seq2(i-1) == seq1(j-1)
            if seq2(i-1) == 'A'
                case1 = F(i-1,j-1) + matchA;
            elseif seq2(i-1) == 'U'
                case1 = F(i-1,j-1) + matchU;
            end
        else
...
aligner = zeros(size(new_seq1));%repmat(' ',size(new_seq1));
aligner(new_seq1 ~= '-' & new_seq2 ~= '-') = 1;%'|'; % add the vertical line for each match and mismatch
alignment1 = new_seq1;
alignment2 = new_seq2;
score = F(end,end);
